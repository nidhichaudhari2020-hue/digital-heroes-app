...
export function DashboardClient({ userId, name }: { userId: string; name: string }) {
  ...
  async function load() {
    ...
    try {
      const [scoreResult, charityResult, choiceResult, subscriptionResult, drawResult, winnerResult, entryResult, donationResult] = await Promise.all([
        ...
        supabase.from("subscriptions").select("plan,status,current_period_end,created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(1).maybeSingle(),
  ...
  async function saveCause() {
    if (!active) {
      tell("Choose an active membership before setting your charity contribution.");
      return;
    }
    if (!choice || percentage < 10 || percentage > 100) {
      tell("Choose a cause and set a contribution between 10% and 100%.");
      return;
    }
    const { error } = await supabase.from("member_charities").upsert({ user_id: userId, charity_id: choice, contribution_percent: percentage });
    if (error) tell(error.message);
  ...
  return <>
    <div className="dashboard-grid">
      <article className="panel"><small>MEMBERSHIP</small><h3>{active ? `${subscription?.plan || "Membership"} plan` : "Not active"}</h3><p>{active && subscription?.current_period_end ? `Renews ${dateTimeLabel(subscription.current_period_end)}` : "Choose a plan to unlock member features."}</p><b>{active ? "ACTIVE MEMBERSHIP" : "ACTION REQUIRED"}</b></article>
    ...
    <div className="summary-grid">
      <section className="dashboard-section"><div className="dashboard-section__head"><div><p className="eyebrow">CHARITY CONTRIBUTION</p><h2>Direct your impact</h2></div></div><div className="choice-grid"><label>Choose a charity<select value={choice} onChange={(event) => setChoice(event.target.value)} disabled={!active}><option value="">Select a cause</option>{charities.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Contribution %<input value={percentage} onChange={(event) => setPercentage(Number(event.target.value))} type="number" min="10" max="100" disabled={!active} /></label><button className="button" type="button" onClick={saveCause} disabled={!active}>Save choice</button></div></section>
