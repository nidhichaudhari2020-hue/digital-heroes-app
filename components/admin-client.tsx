...
export function AdminClient() {
  ...
  const prizeTotal = draws.filter((item) => item.status !== "closed").reduce((sum, item) => sum + Number(item.prize_pool), 0);
  ...
  async function publishDraw(draw: Draw) {
    ...
    setBusy(true);
    try {
      const winning = simulation?.drawId === draw.id && simulation.numbers.length === 5 ? simulation.numbers : await generateFor(draw);
